...
main:
    ...
    # Clear currentSubset once
    la $t2,currentSubset
    li $t3,0
    lw $t4,numVertices
    
# clearing currentSubset[] once before starting the recursion
clear_subset:
...
subset_cleared:
    # Start the search from "before vertex 0" with size 0
    li $a0,-1          # start from before vertex 0
    li $a1,0           # current size = 0
    jal findMaxClique

    j print_result     # go to printing the result
...
isClique:
    ...
    li $s0,0           # i index
# loop over every pair (i, j) to see if all vertices connect
isClique_outer:
    addi $t0,$s2,-1
    ...
    addi $s3,$s0,1     # j = i+1
# grab vertex j from currentSubset & check if there's an edge between i and j in adjMatrix
isClique_inner:
...
# restore everything before returning from recursion
findMax_done:
    lw $ra,32($sp)
    lw $s0,28($sp)
    lw $s1,24($sp)
